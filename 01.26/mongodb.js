...
router.route('/member/edit').post((req, res) => {
    ...
    if(database) { //1. 데이터베이스 연결여부
        //2. 함수호출
        editMember(database, userid, userpw, username, age, (err, result) => {
            if(!err) { //3. 함수호출
                if(result.modifiedCount > 0) { //4. 함수정리
                    res.writeHead('200', {'content-type':'text/html;charset=utf8'});
                ...
                res.write('<p>서버에 오류 발생! 정보 수정에 실행했습니다.</p>');
                res.end();
            } 
        });
    }else {
        ...
        res.write('<p>mongodb 데이터베이스 연결하지 못했습니다.</p>');
        res.end();
    }
});
    ...
    if(database) {
        deleteMember(database, userid, (err, result) => {
            if(!err) {
                ...
                res.write('<p>서버연결오류발생! 회원 정보 삭제에 실패했습니다.</p>');
                res.end();
            }
        });
    }else {
...
const editMember = function (database, userid, userpw, username, age, callback) {
    console.log(`editMember 호출!`); //-> 함수호출확인
    const members = database.collection('member'); // 컬렉션지정(테이블지정);

    //members.updateOne()실행했을때 modifiedCount값이 발생이 됩니다
    members.updateOne({userid: userid}, {$set: {userid: userid, userpw: userpw, username: username, age: age}}, (err, result) => {
        if(!err) { //3-1. member.updateOne() 예외처리
...
const deleteMember = function (database, userid, callback) {
    ...
    //members.deleteOne() 실행시 deletedCount값이 발생
